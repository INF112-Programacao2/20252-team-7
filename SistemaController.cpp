...
std::string SistemaController::obterCPFUnicoCatador() {
    while (true) {
        try {
            ...
            // Se o usuario quiser sair da sessao de cadastro digitando o simbolo, retornamos string vazia
            if (cpf.empty()) {
                std::cout << "Cadastro de CPF cancelado pelo usuario." << std::endl;
                return std::string("");
            }
    ...
    do {
        try {
            ...
            if (opInicial == 1) {
                ...
                if (cpf.empty()) {
                    std::cout << "Entrada de CPF cancelada." << std::endl;
                } else if (catadorExiste(cpf)) {
                ...
                if (cpf.empty()) {
                    // Usuario cancelou insercao do CPF
                    continue; 
                }
                ...
                do {
                    try{
                        ...
                        if(op == 1) coop.relatorioMaterialComprado();
                            ...
                            if (cpfCatador.empty()) {
                                std::cout << "Operacao de registro de compra cancelada." << std::endl;
                                continue; // volta ao menu da cooperativa
                            }
